...
class RegistrationsPage {
	...
	private function render_list( string $area ): void {
		...
		<div class="wrap dms-wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html( $titles[ $area ] ); ?></h1>
			<?php $this->export_buttons( $area, $query ); ?>
			<hr class="wp-header-end">
			<?php if ( 'bin' === $area ) : ?>
				<p class="description"><?php esc_html_e( 'Disapproved registrations stay in the Bin for 30 days from the disapproval date and are then permanently deleted automatically.', 'dms' ); ?></p>
			<?php endif; ?>
	...
	private function render_record( int $id, string $area ): void {
		...
		<div class="wrap dms-wrap dms-record">
			<p><a href="<?php echo esc_url( AdminMenu::area_url( $area ) ); ?>">&larr; <?php esc_html_e( 'Back to list', 'dms' ); ?></a></p>
			<h1>
				<?php
			...
			<div class="dms-record-grid">
				<div class="dms-card">
					<h2><?php esc_html_e( 'Applicant Information', 'dms' ); ?></h2>
					<?php
					$this->dl(
						array(
					...
					?>
					<h2><?php esc_html_e( 'Organization', 'dms' ); ?></h2>
					<?php $this->dl( array( __( 'Organization', 'dms' ) => $r->organization ) ); ?>
					<h2><?php esc_html_e( 'Electoral Information', 'dms' ); ?></h2>
					<?php
					$this->dl(
						array(
							__( 'Region', 'dms' )          => $el->find( ElectoralLevel::REGION, (int) $r->region_id )->name ?? '',
							__( 'Constituency', 'dms' )    => $el->find( ElectoralLevel::CONSTITUENCY, (int) $r->constituency_id )->name ?? '',
							__( 'Polling Station', 'dms' ) => $el->find( ElectoralLevel::POLLING_STATION, (int) $r->polling_station_id )->name ?? '',
						)
					);
					?>
				</div>
				...
				<div class="dms-card">
					<h2><?php esc_html_e( 'Workflow', 'dms' ); ?></h2>
					<?php
					$this->dl(
						array(
					...
					?>
					<h2><?php esc_html_e( 'Actions', 'dms' ); ?></h2>
			...
			<?php if ( $status->is_editable() && current_user_can( 'registrations.edit' ) ) : ?>
				<div class="dms-card">
					<h2><?php esc_html_e( 'Edit Registration', 'dms' ); ?></h2>
			...
			<div class="dms-card">
				<h2><?php esc_html_e( 'History', 'dms' ); ?></h2>
				<?php $this->history( $id ); ?>
			</div>
		</div>
		<?php
	...
	private function actions( object $r, Status $status, StateMachine $machine, bool $is_mine ): void {
		$id   = (int) $r->id;
		$any  = false;
		$form = static function ( string $op, string $label, string $css = 'button', string $extra = '' ) use ( $id ): void {
			printf(
				'<form method="post" action="%1$s" class="dms-action-form">%2$s<input type="hidden" name="registration_id" value="%3$d"><input type="hidden" name="op" value="%4$s">%5$s<button type="submit" class="%6$s">%7$s</button></form>',
				esc_url( admin_url( 'admin-post.php' ) ),
		...
		if ( $is_mine && Status::UNDER_REVIEW === $status ) {
			if ( current_user_can( 'review.approve' ) ) {
				$form( 'approve', __( 'Approve', 'dms' ), 'button button-primary' );
				$any = true;
			}
